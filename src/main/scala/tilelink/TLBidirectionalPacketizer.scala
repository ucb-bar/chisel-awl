...
class TLBidirectionalPacketizer[S <: DecodedSymbol](clientEdge: TLEdgeOut, managerEdge: TLEdgeIn, decodedSymbolsPerCycle: Int, symbolFactory: () => S)(implicit val p: Parameters)
    ...
    def cProbeAck(c: TLBundleC) = (c.opcode === TLMessages.ProbeAck) || (c.opcode === TLMessages.ProbeAckData)
    def cRelease(c: TLBundleC) = (c.opcode === TLMessages.Release) || (c.opcode === TLMessages.ReleaseData)
    cOutstanding := cOutstanding + Mux(tltx.b.fire() && txBFirst, tlResponseMap(tltx.b.bits), 0.U) - Mux(tlrx.c.fire() && cProbeAck(tlrx.c.bits), tlrx.cEdge.last(tlrx.c), 0.U)
    dOutstanding := dOutstanding + Mux(tltx.a.fire() && txAFirst, tlResponseMap(tltx.a.bits), 0.U) + Mux(tltx.c.fire() && cRelease(tltx.c.bits) && txCFirst, tlResponseMap(tltx.c.bits), 0.U) - Mux(tlrx.d.fire(), tlrx.dEdge.last(tlrx.d), 0.U)
